...
import os
import glob
from libs.tools.opts import parse_opts
from libs.tools.cfg import parse_cfg
from torch.utils.data import Dataset
from torchvision import transforms
from scipy.io import loadmat
from libs.models.model import YOWO
from libs.tools.utils import *
from libs.tools.eval_results import *
import os.path as osp
from libs.models.model_slowfast import YOWO_slowfast

opt = parse_opts()

save_dir = osp.join("logs/test_logs", opt.dataset, opt.backbone_2d + "_" + opt.backbone_3d + "_" + opt.check_name)
...
gt_file       = 'configs/finalAnnots.mat' # Necessary for ucf
...
#testlist      = os.path.join(base_path + "splits", 'testlist_video.txt')
testlist = opt.test_video_list
...
# Create model
if opt.backbone_3d == 'slowfast':
    model = YOWO_slowfast(opt)
else:
    model = YOWO(opt)
...
def video_mAP_jhmdb():
    ...
    for line in lines:
        ...
        line = line.rstrip()
        print(os.path.join(base_path, 'rgb-images', line))
        test_loader = torch.utils.data.DataLoader(
                          testData(os.path.join(base_path, 'rgb-images', line),
                          shape=(224, 224), transform=transforms.Compose([
                          transforms.ToTensor()]), clip_duration=clip_duration),
        ...
        for batch_idx, (data, target, img_name) in enumerate(test_loader):
            ...
            with torch.no_grad():
                ...
                output = model(data).data
                all_boxes = get_region_boxes_video(output, conf_thresh, num_classes, anchors, num_anchors, 0, 1)
                #all_boxes_visual = get_region_boxes_video_visual(output, conf_thresh, num_classes, anchors, num_anchors, 0, 1)
                if opt.is_vis:
                    all_boxes_visual = get_region_boxes_video_visual(output, conf_thresh, num_classes, anchors, num_anchors, 0, 1)
                for i in range(output.size(0)):
                    boxes = all_boxes[i]
                    ...
                    ####
                    if opt.is_vis:
                        boxes_visual = all_boxes_visual[i]
                        boxes_visual = nms(boxes_visual, nms_thresh)
                        if not os.path.exists(os.path.join(save_dir, path_split[1])):
                            os.mkdir(os.path.join(save_dir, path_split[1]))
                        plot_boxes_tensor(data.cpu(), boxes_visual, os.path.join(save_dir, path_split[1], path_split[2]), CLASSES)
                        #plot_boxes_classes_tensor(data.cpu(), boxes, cls_max_confs, cls_max_ids, os.path.join(save_dir, path_split[1], path_split[2]),CLASSES)
                    ####
                    n_boxes = len(boxes)
    ...
    for iou_th in iou_list:
        ...
        class_ap, total_ap = evaluate_videoAP(gt_videos, detected_boxes, CLASSES, iou_th, True)
        d = dict(zip(CLASSES, class_ap))
        print('avg mAP:', total_ap)
        print('mAP per class:', d)

        f = open(os.path.join(save_dir, str(iou_th) + '.txt'), 'a')
        f.write("Total mAP: " +  str(total_ap))
        for i in d:
            f.write("\r" + i + ": " + str(d[i]))
        f.close()
