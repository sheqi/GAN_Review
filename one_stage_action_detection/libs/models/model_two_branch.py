...
from libs.models.backbones_3d import mobilenet, shufflenet, mobilenetv2, shufflenetv2, resnext, resnet
...
class YOWO(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.cfam_region = CFAMBlock(num_ch_2d+num_ch_3d, 1024)
        self.cfam_class = CFAMBlock(num_ch_2d+num_ch_3d, 1024)

        self.conv_final_class = nn.Conv2d(1024, 5*opt.n_classes, kernel_size=1, bias=False)
        self.conv_final_region = nn.Conv2d(1024, 5*(4+1), kernel_size=1, bias=False)
    ...
    def forward(self, input):
        ...
        x_r = self.cfam_region(x)
        x_c = self.cfam_class(x)
        out_r = self.conv_final_region(x_r)
        out_c = self.conv_final_class(x_c)
        b, _, h, w = out_r.size()
        b, _, h, w = out_c.size() 
        out_r = out_r.view(b, 5, -1, h, w)
        out_c = out_c.view(b, 5, -1, h, w)
        out = torch.cat((out_c, out_r), dim=2)
        out = out.view(b, 5*(self.n_classes + 4 + 1), h, w)
