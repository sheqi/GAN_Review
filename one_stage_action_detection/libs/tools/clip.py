...
def load_data_detection(base_path, imgpath, train, train_dur, shape, dataset_use='ucf101-24', jitter=0.2, hue=0.1, saturation=1.5, exposure=1.5):
    ...
    if dataset_use == 'ava':
        labpath = os.path.join(base_path, 'labels', im_split[0] + '/' + '{:06d}.txt'.format(im_ind))
    else:
    ...
    #while not os.path.exists(os.path.join(base_path, 'rgb-images', im_split[0], im_split[1] ,'{:05d}.png'.format(max_num))):
    #    print(os.path.join(base_path, 'rgb-images', im_split[0], im_split[1] ,'{:05d}.png'.format(max_num)))
    #    print(os.path.exists(os.path.join(base_path, 'rgb-images', im_split[0], im_split[1] ,'{:05d}.png'.format(max_num))))
    #    max_num = max_num - 1
    ...
    for i in reversed(range(train_dur)):
        ...
        elif dataset_use =='ava':
            path_tmp = os.path.join(base_path, 'rgb-images', im_split[0], im_split[0] + "_" + '{:05d}.jpg'.format(i_temp))
        else:
            path_tmp = os.path.join(base_path, 'rgb-images', im_split[0], im_split[1] ,'{:05d}.png'.format(i_temp))
        #print(i_temp, max_num)
        clip.append(Image.open(path_tmp).convert('RGB'))
        #index_list.append(i_temp)
    #print(im_ind, ":", index_list)
    ...
    #print(im_split[0] + '_' +im_split[1] + '_' + im_split[2])
    if train:
        return clip, label
    else:
