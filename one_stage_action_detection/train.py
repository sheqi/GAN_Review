...
import libs.tools.dataset as dataset
import random
import math
import os
...
from libs.losses.region_loss import RegionLoss

from libs.models.model import YOWO, get_fine_tuning_parameters
from libs.models.model_slowfast import YOWO_slowfast, get_fine_tuning_parameters_slowfast
...
save_dir = osp.join("logs/train_logs", opt.dataset, opt.backbone_2d + "_" + opt.backbone_3d + "_" + opt.check_name)
...
backupdir     = osp.join(save_dir, 'backup')
...
if opt.backbone_3d == 'slowfast':
    parameters = get_fine_tuning_parameters_slowfast(model, opt)
else:
    ...
    for batch_idx, (frame_idx, data, target) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            for i in range(output.size(0)):
                ...
                detection_path = os.path.join(save_dir, 'detections', 'detections_'+str(epoch), frame_idx[i])
                current_dir = os.path.join(save_dir, 'detections', 'detections_'+str(epoch))
                if not os.path.exists(os.path.join(save_dir, 'detections')):
                    os.mkdir(os.path.join(save_dir, 'detections'))
