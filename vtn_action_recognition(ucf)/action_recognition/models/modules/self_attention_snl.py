...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        # flops: < 20k
        attn = (attn + attn.permute(0, 2, 1)) / 2
...
class MultiHeadAttention_snl(nn.Module):
    """ Multi-Head Attention module """

    def __init__(self, n_head, input_size, output_size, d_k, d_v, dropout=0.1, use_proj=True, layer_norm=True):
        """
        Args:
        ...
        """
        super(MultiHeadAttention_snl, self).__init__()
        ...
        self.w_snl = nn.Parameter(torch.FloatTensor(n_head, input_size, d_v))
        ...
        self.proj_2 = Linear(n_head * d_v, output_size)
        ...
        init.xavier_normal_(self.w_snl)
    ...
    def forward(self, q, k, v):
        ...
        #####################################
        snl_s = torch.bmm(snl_s, self.w_snl).view(-1, len_v, d_v)  # (n_head*mb_size) x len_v x d_v
        outputs = outputs + snl_s
        #####################################
        #outputs = outputs + v_s

        split_size = mb_size.item() if isinstance(mb_size, torch.Tensor) else mb_size
        v_s = unsquash_dim(outputs, 0, (-1, split_size))
        v_s = v_s.permute(1, 2, 0, 3).contiguous().view(split_size, len_q, -1)
...
class DecoderBlock_snl(nn.Module):
    """ Compose with two layers """

    def __init__(self, input_size, hidden_size, inner_hidden_size, n_head, d_k, d_v, dropout=0.1, layer_norm=True):
        super(DecoderBlock_snl, self).__init__()
        # flops: 17.5m
        self.slf_attn = MultiHeadAttention_snl(n_head, input_size, hidden_size, d_k, d_v, dropout=dropout,
