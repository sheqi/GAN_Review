...
from .modules.self_attention_snl import DecoderBlock_snl, PositionEncoding
from ..utils import get_fine_tuning_parameters, load_state


class VideoTransformer_snl(nn.Module):
...
class VideoTransformerEncoder(VideoTransformer_snl):
...
class VideoTransformerDecoder(VideoTransformer_snl):
...
class SelfAttentionDecoder(nn.Module):
    def __init__(self, input_size, hidden_size, n_heads, sequence_size, inner_hidden_factor=2, layer_norm=True):
        ...
        self.layers = nn.ModuleList([
            DecoderBlock_snl(inp_size, hid_size, hid_size * inner_hidden_factor, n_head, hid_size // n_head,
